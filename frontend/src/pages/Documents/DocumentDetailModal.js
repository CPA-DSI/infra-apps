...
const DocumentDetailModal = ({ show, onClose, doc, relationData, onEdit }) => {
  ...
  // File type-based sections
  const renderFileTypeSection = () => {
    const mime = (doc.type_mime || '').toLowerCase();
    const isPdf = mime.includes('pdf');
    const isImage = mime.includes('image') || mime.includes('png') || mime.includes('jpg') || 
                   mime.includes('jpeg') || mime.includes('gif');
    const isWord = mime.includes('word') || mime.includes('docx');
    const isExcel = mime.includes('excel') || mime.includes('xlsx') || mime.includes('xls');
        ...
        <div className="doc-details-section" style={{
          ...
          <div style={{ marginBottom: '16px' }}>
            {doc.chemin_stockage ? (
              ...
              <span className="text-muted">Aucun fichier disponible</span>
            )}
          </div>
        </div>
      );
      ...
      return (
        <div className="doc-details-section" style={{
          ...
          <div style={{ textAlign: 'center', marginBottom: '16px' }}>
            {doc.chemin_stockage ? (
              <img 
                src={getDownloadUrl(doc.chemin_stockage)}
                alt={doc.nom_fichier}
                style={{ 
                  maxWidth: '100%', 
                  maxHeight: '60vh', 
              ...
              <div style={{ padding: '40px', background: '#f8fafc', borderRadius: '8px' }}>
                <FaFileImage style={{ fontSize: '3rem', color: '#cbd5e1' }} />
    ...
    // Office Section (Word/Excel)
    if (isWord || isExcel) {
      const viewerUrl = isWord 
        ? `https://docs.google.com/gview?embedded=1&url=${window.location.origin}/api/files${doc.chemin_stockage}`
        : `https://docs.google.com/spreadsheets/d/google/docs/viewer?url=${window.location.origin}/api/files${doc.chemin_stockage}&embedded=1`;
            ...
            {doc.chemin_stockage ? (
              <div>
                <iframe 
                  ...
                  title={`Prévisualisation ${isWord ? 'Word' : 'Excel'}`}
                />
                <div className="d-flex gap-2 mt-3">
                  <Button 
                    size="sm" 
                    href={getDownloadUrl(doc.chemin_stockage)}
                    target="_blank" 
